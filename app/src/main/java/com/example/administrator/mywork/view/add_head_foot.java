 ...
 * 作者：wu
 */
public class add_head_foot extends RecyclerView.Adapter<RecyclerView.ViewHolder> {


    /**
     * 首先，使用recycleView与使用Listview原理很类似，
     * 添加头部主要是在AbsListView中添加的RecyclerView.
     * AdapterRecyclerView.ViewHolder中再添加一个内部的adapter.
     * 一个OuterAdapter一个InnerAdapter。
     * 外部Adapter主要是通过本身的headviews/footviews来保存添加进来的头部view与尾部view。
     * getItemType来判断view是不是头部或者尾部，
     * 如果是头部则在外部adapter中处理oncreateView。
     * 如果不是，则是RecycleView中间部分，则交给innerAdapter去处理。
     */
...
//    数据观察者   现在还不会
    ...
    public add_head_foot(RecyclerView.Adapter innerAdapter) {
